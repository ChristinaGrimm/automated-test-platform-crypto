...
class Difference(Root):
    ...
    # def cyclic_Shift_In(self, r):
    #     assert (r >= 1)
    #     return 'cyclic_input_r' + str(r)
    #
    # def cyclic_Shift_Out(self, r):
    #     assert (r >= 1)
    #     return 'cyclic_output_r' + str(r)
    ...
    # def cyclic_left_shift(self, input, output, num):
    #     constraint = []
    #     if num == 0:
    #         constraint += [f"ASSERT({'{0} = {1}'.format(input, output)});"]
    #     else:
    #         constraint += [f"ASSERT("
    #                        f"{'{0} = '.format(output) + '{0}[{1}:0] @ {0}[{2}:{3}]'.format(input, self.Blocksize//self.Branch_number-num-1, self.Blocksize//self.Branch_number - num)}"]
    #
    #
    #     return constraint
    ...
    #     if self.Sbox_bit != 8:
    #         # constraint = [
    #         #     "ASSERT(NOT(S[{'0bin{0}@{1}@'.format('0' * (8 - self.Sbox_bit), inS) + '0bin{0}@{1}'.format('0' * (8 - self.Sbox_bit), outS)}) = 0bin0));"]
    #         constraint = [  # 问题：Sbox_bit不能超过8吗？懂了
    #             f"ASSERT(NOT(S["
    #             f"{'0bin{0}@{1}@'.format('0' * (8 - self.Sbox_bit), inS) + '0bin{0}@{1}'.format('0' * (8 - self.Sbox_bit), outS)}] = 0bin0));"]
    #         self.sum.append('|'.join(['{0}[{1}:{1}]'.format(inS, i) for i in range(self.Sbox_bit)])) #这个sum让S盒的每一个比特都用"或"相连，意思就是如果有一个非0，则这个S盒就是活跃的
    #
    #     else:
    #         constraint = [f"ASSERT(NOT(S[{'{0}@'.format(inS) + '{0}'.format(outS)}] = 0bin0));"]
    #         self.sum.append('|'.join(['{0}[{1}:{1}]'.format(inS, i) for i in range(self.Sbox_bit)]))
    #
    #     return constraint
    ...
    def ZonggeshuConstraint(self, inS):  # 这里让sum1也加上去
        ...
        # if self.Sbox_bit != 8:
        #     self.sum1.append('|'.join(['{0}[{1}:{1}]'.format(inS, i) for i in range(self.Sbox_bit)]))
        #     #这个sum让S盒的每一个比特都用"或"相连，意思就是如果有一个非0，则这个S盒就是活跃的
        #
        # else:
        #     self.sum1.append('|'.join(['{0}[{1}:{1}]'.format(inS, i) for i in range(self.Sbox_bit)]))
